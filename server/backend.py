...
from server.model import AnalysisRequest, AnalysisResponse, QueryRequest, QueryResponse
...
async def analyze_issue(request: AnalysisRequest):
    ...
    try:
        if _agent is None:
            raise HTTPException(status_code=503, detail="Agent not initialized. Please check Ollama and database connection.")
        
        logger.info(f"Analyzing issue: {request.owner}/{request.repo}#{request.issue_id}")
        
        # Fetch issue from GitHub
        try:
            issue = get_issue_by_issue_id(f"{request.owner}/{request.repo}", request.issue_id)
...
async def build_rag_for_repo(request: AnalysisRequest):
    ...
    try:
        ...
        if not documents:
            raise HTTPException(status_code=400, detail="No documents retrieved from repository")
